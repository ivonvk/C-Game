...
void inGame() {
	if (inGameUI.Draw_iPlayerHP <= 0) {
		Scene = 2;
		MB.btnTimer = 20;
		inGameUI.isWin = false;
	}
		...
		if (objItem[i].isActive) {
			if (abs(objItem[i].x - player.x) < 42 && abs(objItem[i].y - player.y) < 42) {
				...
				case 2:
					...
					LeftDoor += 200;
					RightDoor += 200;
					if (TopDoor > 5000 ? TopDoor = 5000  : TopDoor += 0);
					if (BottomDoor > 5000 ? BottomDoor = 5000 : BottomDoor += 0);
					if (LeftDoor > 5000 ? LeftDoor = 5000 : LeftDoor += 0);
					if (RightDoor > 5000 ? RightDoor = 5000 : RightDoor += 0);
					objItem[i].isActive = false;
					break;
				}
			}
...
int main(int argc, char **argv) {
    ...
    // create the window
    glutCreateWindow("DEEP SUICIDE");

	HWND hwnd = FindWindow(NULL, "DEEP SUICIDE");
